...
stopwords = list(text.ENGLISH_STOP_WORDS.union(['including', 'interview', 'like', 'rev', 'ms', 'mrs', 'dr', 'sherry',
                                                ...
                                                'jerkins', 'lincoln', 'she', 'her', 'hers', 'he', 'him', 'his',
                                                'africanamericanoralhistory']).union(names))
...
hdbscan_model = HDBSCAN(min_cluster_size=5,  min_samples=2, metric='euclidean', cluster_selection_method='leaf', prediction_data=True)

representation_model = KeyBERTInspired()

vectorizer_model = CountVectorizer(stop_words=stopwords,  ngram_range=(1, 2), min_df=1)

embedding_model = SentenceTransformer("sentence-transformers/all-mpnet-base-v2")

ctfidf_model = ClassTfidfTransformer( reduce_frequent_words=True, bm25_weighting=True, seed_words=candidate_seed_topics, seed_multiplier=100)

topic_model = (BERTopic(embedding_model=embedding_model, hdbscan_model=hdbscan_model, umap_model=umap_model,
                        representation_model=representation_model, seed_topic_list=candidate_seed_topics, vectorizer_model=vectorizer_model,
                        ctfidf_model=ctfidf_model, verbose=True, nr_topics='auto', top_n_words=5))
...
distro = topic_model.visualize_distribution(topic_distr[1])
distro.show()

topic_model.save("single_doc_model", serialization="safetensors", save_ctfidf=True, save_embedding_model=embedding_model)